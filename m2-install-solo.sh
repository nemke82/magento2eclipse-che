...
cd magento2 && cp -avr .* /projects/magento2che;
cd /projects/magento2che && rm -r -f magento2;

mysql -u root -pnem4540 -e 'create database nemanja;';
cd /projects/magento2che;
composer install;
php bin/magento setup:install --db-name='nemanja' --db-user='root' --db-password='nem4540' --base-url="https://serverg7snjxwd-magento2che-server-8080.192.168.34.100.nip.io/" --backend-frontname='admin' --admin-user='admin' --admin-password='adm4540' --admin-email='ne@nemanja.io' --admin-firstname='Nemanja' --admin-lastname='Djuric' --use-rewrites='1' --use-secure='1' --base-url-secure="https://serverg7snjxwd-magento2che-server-8080.192.168.34.100.nip.io/" --use-secure-admin='1' --language='en_US' --db-host='127.0.0.1' --cleanup-database --timezone='America/New_York' --currency='USD' --session-save='redis' &&
